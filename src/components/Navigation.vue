<template>
  <nav class="sticky top-0 z-[100] w-full border-b border-[rgba(46,7,63,0.14)] bg-[rgba(247,247,247,0.96)] shadow-[0_8px_20px_rgba(38,38,38,0.06)] backdrop-blur-md">
    ...
    <div class="relative flex w-full items-center justify-between px-4 py-3 sm:px-6 lg:px-8">

      <!-- LEFT: LOGO -->
      <div class="flex items-center gap-3">
        <img :src="navLogo" alt="Lutowl logo"
      ...
      <!-- CENTER: FIXED POSITION (DOES NOT MOVE) -->
      <ul
        class="absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 lg:flex items-center gap-6"
      >
        <li>
          <router-link to="/" class="nav-link" active-class="nav-active">Home</router-link>
        </li>
        <li>
          <router-link to="/discover" class="nav-link" active-class="nav-active">Discover</router-link>
        </li>
        <li>
        ...
        <li v-if="user">
          <router-link to="/user/dashboard" class="nav-link" active-class="nav-active">
            Dashboard
          </router-link>
        </li>
      </ul>
      ...
      <div class="flex items-center gap-3 relative">
        ...
        <div v-if="!user" class="flex gap-2">
          <router-link to="/login" class="auth-btn-outline">Log In</router-link>
